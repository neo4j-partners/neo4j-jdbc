 ...
 * @author Ryan Knight
 */
class AliasRegistryTests extends QomTestSupport {
	...
	class StructuralLookupTests {
		...
		void lookupCountStar() {
			...
			this.registry.register(unwrapAlias(countField), getAliasName(countField));
		...
		void lookupSumDifferentColumn() {
			...
			this.registry.register(unwrapAlias(sumField), getAliasName(sumField));
		...
		void lookupMultipleAggregates() {
			...
			for (var sf : selectFields) {
				this.registry.register(unwrapAlias(sf), getAliasName(sf));
			}
	...
	class RoundTripTests {
		...
		void countStarRoundTrip() {
			...
			for (var sf : select.$select()) {
				registry.register(unwrapAlias(sf), getAliasName(sf));
			}
